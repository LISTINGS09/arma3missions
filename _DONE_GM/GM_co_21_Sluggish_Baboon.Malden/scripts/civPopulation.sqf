...
ZCS_fnc_SpawnHunter = {
	...
	if (random 1 <= 0.9) then {
		uniformContainer _hunter addMagazineCargo [_ammo, 5];
...
ZCS_fnc_SpawnBomber = {
	...
	[_bomber] spawn {
		...
		waitUntil { 
			sleep 1; 
			playSound3D ["A3\sounds_f\sfx\beep_target.wss", _unit, false, getPosASL _unit, 1, 0.5, 100];
			(!alive _unit || allPlayers findIf { alive _x && _unit distance _x < 5 } >= 0) 
		};
