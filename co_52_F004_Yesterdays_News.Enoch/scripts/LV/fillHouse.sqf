...
_nul = [thisTrigger, "NATO"] execVM "scripts\LV\fillHouse.sqf";
...
_factionsFound = [];
{
	_factionsFound pushBackUnique [
		...
		[east, west, independent] select getNumber (_x >> "side")
		getText (_x >> "faction"),
		getText (_x >> "editorSubcategory")
	];
} forEach ("(configName _x iskindOf 'CAManBase') && (configFile >> "CfgVehicles"));
