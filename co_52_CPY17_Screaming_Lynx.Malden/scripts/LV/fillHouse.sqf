...
for "_i" from 1 to _rat do {
	...
	for "_dir" from (getDir _unitBld) to ((getDir _unitBld) + 359) step 45 do { 
		// Check 3m
		if (count (lineIntersectsWith [_unitEyePos, ((_unitEyePos getPos [3, _dir]) vectorAdd [0, 0, _unitEyePos select 2])] select {_x isKindOf 'Building'}) > 0) then { 
			_p4 pushBack _dir;
			_helper = "Sign_Arrow_Direction_F";
		} else { 
			// Check 10m
			if (count (lineIntersectsWith [_unitEyePos, ((_unitEyePos getPos [10, _dir]) vectorAdd [0, 0, _unitEyePos select 2])] select {_x isKindOf 'Building'}) > 0) then { 
				_p3 pushBack _dir;
