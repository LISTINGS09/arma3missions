...
if (side group player != CIVILIAN) then {
	...
	<br/>We have received intelligence that two of our fellow soldiers have been captured by AAF forces and are being held as prisoners of war. It is our duty to rescue them as soon as possible.
	<br/>
	<br/>Our intelligence suggests that the prisoners are being held in a heavily guarded compound located in a remote area, with one checkpoint leading up to it. The AAF forces have established a strong defensive perimeter around the compound, including watchtowers, guard posts, and patrols. An airborne QRF is likely from the surrounding airports.
	<br/>
	<br/>Your primary objective is to rescue the prisoners and bring them back home safely.
	<br/>
	<br/>%1 <!--- Casualties Cap --->
	...
	<br/>%2%3
	<br/>
	<br/>Additional airborne QRF forces are expected, type unknown. 
	<br/>
	...
	(((vehicles select { side _x getFriend side group player < 0.6 && !((_x isKindOf "staticWeapon" || _x isKindOf "static") && !("Artillery" in getArray (configFile >> "CfgVehicles" >> typeOf _x >> "availableForSupportTypes"))) && count crew _x > 0}) apply {  getText (configFile >> "CfgVehicles" >> typeOf _x >> "displayName") }) call BIS_fnc_consolidateArray) apply { format["%2x <font color='#00FFFF'>%1</font><br/>", _x#0, _x#1] } joinString "",
	...
	if (_icon != "" && count (allMapMarkers select { (getPos _veh distance2D getMarkerPos _x) < 25 }) == 0) then {
