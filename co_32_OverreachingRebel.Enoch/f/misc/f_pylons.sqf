...
fnc_pylon_setPylons = {
	...
	
	{
		...
		[_v, [_forEachIndex + 1, if (_forEachIndex >= count _loadout) then { "" } else { _loadout#_forEachIndex }, true, _turret]] remoteExec ["setPylonLoadout"]; // Local command, but safest to run on all machines?
	} forEach _pylons;
