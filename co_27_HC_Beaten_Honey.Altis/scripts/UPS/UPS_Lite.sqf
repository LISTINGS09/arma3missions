...
// Enable Support Provider
if (_isArty) then {
	missionNamespace setVariable [format["ZAI_%1_HasSupport", side _grp], true];
};
...
while {TRUE} do {
	...
	if (_grpLeader distance2D _foundEnemy < _shareDist) then {
		...
		if ({ _x distance _attackPos < _safeDist } count (missionNamespace getVariable [format["ZAI_%1_ArtyQueue", side _grp],[]]) == 0 && (missionNamespace getVariable [format["ZAI_%1_HasSupport", side _grp], false])) then { 
