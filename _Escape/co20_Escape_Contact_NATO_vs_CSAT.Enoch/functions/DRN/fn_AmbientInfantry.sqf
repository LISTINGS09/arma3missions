...
while {true} do {
    ...
    while {count _activeGroups < _groupsCount} do {
        ...
        _faction = selectRandom [A3E_VAR_Side_Ind , A3E_VAR_Side_Opfor];
        ...
        _group = createGroup [_faction, true];
        
        for [{_i = 0}, {_i < _unitsInGroup}, {_i = _i + 1}] do {
			_group createUnit [selectRandom _possibleInfantryTypes, _spawnPos, [], 0, "FORM"];
        };
