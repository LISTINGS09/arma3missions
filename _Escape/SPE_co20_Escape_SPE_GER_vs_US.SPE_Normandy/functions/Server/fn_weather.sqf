...
if (_time < 1) then { forceWeatherChange };

_time = 60 * (30 + random 30);

diag_log format["[ESCAPE]: Weather sleeping for: %1 mins", round (_time / 60)];

sleep (if (_weatherParamFog > 0) then { _time * 0.5 } else { _time });

[_time] spawn A3E_fnc_weather;