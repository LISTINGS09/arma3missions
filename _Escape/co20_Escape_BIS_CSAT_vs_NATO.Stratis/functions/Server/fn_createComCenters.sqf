...
private _instanceNo = 0;
{
	...
	if _ok then {
		...
		[_pos, _dir, a3e_arr_ComCenStaticWeapons, a3e_arr_ComCenParkedVehicles] call selectRandom _ComCenterTemplates;
	...
	if (_instanceNo >= A3E_ComCenterCount) exitWith {};
} forEach _shuffledComCenterMarkers;

{ [_x, 80, selectRandom[A3E_VAR_Side_Opfor], "COMCENTER"] call A3E_fnc_initLocationZone } foreach _comCenPositions;
[_comCenPositions, A3E_Param_EnemySpawnDistance, A3E_Param_EnemyFrequency] call drn_fnc_Escape_InitializeComCenArmor;

missionNamespace setVariable ["a3e_var_Escape_communicationCenterPositions", _comCenPositions, true];

//["drn_CommunicationCenterPatrolMarker", A3E_VAR_Side_Opfor, "INS", 4, A3E_Param_EnemySpawnDistance, A3E_Debug] call drn_fnc_InitGuardedLocations;
