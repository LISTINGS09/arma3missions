params[["_overWriteBase",-1],["_overWriteMod",-1],["_min",2],["_max",8]];

if (count allUnits > 150) exitWith { 2 };

private _base = missionNameSpace getVariable ["A3E_Param_EnemyGroupSize",5];

if(_overWriteBase >= 0) then { _base  = _overWriteBase };

private _mod = missionNameSpace getVariable ["A3E_Param_DynamicGroupSizeMultiplier",1];
if(_overWriteMod >= 0) then { _mod  = _overWriteMod };
