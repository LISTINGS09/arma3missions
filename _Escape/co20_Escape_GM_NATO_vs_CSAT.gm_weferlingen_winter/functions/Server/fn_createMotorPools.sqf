...
if (_mapsize < 5000) then { _mpc = 0 } else {
	if (_mapsize < 10000) then { _mpc = 1 } else {
		if (_mapsize < 15000) then { _mpc = 2 } else {
			if (_mapsize < 20000) then { _mpc = 3 } else { 
				if (_mapsize < 25000) then { _mpc = 4 } else { _mpc = 5 };
			};
		};
	};
};
...
private _MotorPoolTemplates = missionNameSpace getVariable ["A3E_MotorPoolTemplates",[A3E_fnc_BuildMotorPool,A3E_fnc_BuildMotorPool1,A3E_fnc_BuildMotorPool2,A3E_fnc_BuildMotorPool3,A3E_fnc_BuildMotorPool4]];

{
	...
	] call selectRandom _MotorPoolTemplates;
