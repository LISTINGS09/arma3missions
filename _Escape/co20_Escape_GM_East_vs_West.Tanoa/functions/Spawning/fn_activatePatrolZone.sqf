...
if(!(_active)) then {
	...
	if(!_initialized) then {
		  ...
		  switch (_spawnCount) do
				{
					...
					case 2: // 3-5 players
					{
					   _patrolsPerSqmSqrt = 0.015;
					};
					case 3: // 6-8 players
					{
					   _patrolsPerSqmSqrt = 0.02;
					};
					default // 6-8 players
					{
						_patrolsPerSqmSqrt = 0.01;
		...
		for "_i" from 1 to (_patrolCount min 8) do {
		
			[format["Activating Zone %1 - Initialising Group #%2 at %4 (%3)",_zoneIndex,_i,_patrolCount,_pos]] call a3e_fnc_debugmsg;
			...
			private _unitCount = if (count allUnits > 150) then { 2 } else { ([] call a3e_fnc_getDynamicSquadSize) min 6 };
		...
		private _grpArray = [];
		{
			...
			[_grp, _marker] call A3E_fnc_Patrol;
			[_grp] spawn A3E_fnc_TrackGroup;
			[format["Activating Zone %1 - Spawning Group #%2 at %4 (%3)",_zoneIndex,_i, count units _grp]] call a3e_fnc_debugmsg;
		} forEach _groups;
