...
if (side group player != CIVILIAN) then {
	...
	<br/>CTRG forces are tasked with intercepting a high-value CSAT cargo truck located near the outskirts of Paros, to the northeast. Intel suggests that the truck contains sensitive materials critical to ongoing CSAT operations in the region. The truck is currently guarded by a small but well-armed detachment of CSAT forces, and the surrounding area is under surveillance. Recent drone reconnaissance has identified a few scattered patrols and a lightly fortified checkpoint on the approach routes.
	<br/>
	<br/>Your objective is to locate, secure, and recover the CSAT cargo truck. Time is critical, as reinforcements from a nearby CSAT outpost may be mobilized if the operation takes too long or if the alarm is raised.
	<br/>
