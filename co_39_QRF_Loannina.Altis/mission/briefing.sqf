...
if (side group player != CIVILIAN) then {
	...
	<br/>Joint Operations Center (JOC) informs there is a good opportunity to destroy and eliminate the two transport trucks located at Loannina village. These trucks are used by the enemy and terrorists to transport drugs and heavy weapons into enemy territory.
	<br/>
	<br/>Destroy the <font color='#0080FF'><marker name='obj1'>transport trucks</marker></font color>.
	<br/>Move to the <font color='#0080FF'><marker name='obj2'>extraction point</marker></font color>.
	<br/>
	<br/>%1 <!--- Casualties Cap --->
	...
	<br/>Created by <font color='#FF0080'>Bravo 6</font color>
	<br/>
...
// Unit (Enemy Markers)
{
	...
	if (_icon != "" && count (allMapMarkers select { (getPos _veh distance2D getMarkerPos _x) < 50 }) == 0 && missionNameSpace getVariable ["f_param_ZMMDiff", 1] < 0) then {
