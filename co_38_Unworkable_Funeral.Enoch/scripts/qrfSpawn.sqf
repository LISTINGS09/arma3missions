...
for [{_wave = 1}, {_wave < _waveMax}, {_wave = _wave + 1}] do {
	...
	switch (_wave) do {
		case 1: {
			...
			[_location, _spawnLand, _side, ((count allPlayers) / 3.5) max 3] call zmm_fnc_spawnUnit;
		...
		case 4: {
			[_location, _spawnRoad, _side, selectRandom (_Light + _Truck)] call zmm_fnc_spawnUnit;t;
			[_location, _spawnLand, _side, ((count allPlayers) / 3.5) max 3] call zmm_fnc_spawnUnit;
		};
		default {
			[_location, _spawnRoad, _side, selectRandom (_Light + _Truck)] call zmm_fnc_spawnUnit;
