...
_Medium = [["I_APC_Wheeled_03_cannon_F","[_grpVeh,['Guerilla_02',1],['showSLATHull',0.3]] call BIS_fnc_initVehicle;"],["I_LT_01_cannon_F","[_grpVeh,['Indep_Olive',1],['showCamonetHull',0.5,'showSLATHull',0.2]] call BIS_fnc_initVehicle;"]];
...
for [{_wave = 1}, {_wave < _waveMax}, {_wave = _wave + 1}] do {
	...
	switch (_wave) do {
		...
		case 4: {
			[_location, _spawnRoad, _side, selectRandom (_Medium + _Truck)] call zmm_fnc_spawnUnit;
			[_location, _spawnLand, _side, selectRandom _Team] call zmm_fnc_spawnUnit;
		};
		default {
