...
private _side = WEST;
ZMM_WESTMan = ["SPE_GER_SquadLead","SPE_GER_mgunner","SPE_GER_rifleman","SPE_GER_medic","SPE_GER_rifleman","SPE_GER_Assist_SquadLead","SPE_GER_rifleman","SPE_GER_amgunner","SPE_GER_rifleman","SPE_GER_LAT_Rifleman","SPE_GER_ober_grenadier","SPE_GER_rifleman"];
...
if (isNil "ZAU_SafeAreas" ) 	then { ZAU_SafeAreas = ["safezone"] }; // List of Safe Areas - Imported from ZMM
	...
	
	{
		...
		if (count ZAU_UnitsActive < ZAU_UnitsMax && random 100 <= ZAU_UnitsChance) then {
			...
			if (random 1 > 0.3) then {
				[_patrolGroup, getPos _bld, 200 + random 100] call BIS_fnc_taskPatrol;
			};
