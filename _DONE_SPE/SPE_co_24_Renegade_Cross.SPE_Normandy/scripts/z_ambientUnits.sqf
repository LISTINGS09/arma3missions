...
ZMM_GUERMan = ["SPE_FFI_TeamLeader_Sten","SPE_FFI_Autorifleman","SPE_FFI_Fighter_Carbine","SPE_FFI_Fighter_MP40","SPE_FFI_Fighter","SPE_FFI_Fighter_Garand","SPE_FFI_Grenadier","SPE_FFI_Tankhunter","SPE_FFI_Tankhunter","SPE_FFI_Fighter_G43","SPE_FFI_Fighter_M3","SPE_FFI_Fighter"];
...
if (isNil "ZAU_UnitsMax" ) 		then { ZAU_UnitsMax = 30 * (missionNamespace getVariable ["f_param_ZMMDiff", 1]) };		// Max units active at once.
if (isNil "ZAU_UnitsChance" ) 	then { ZAU_UnitsChance = 60 }; 	// Overall chance to spawn
if (isNil "ZAU_UnitsGarrison" ) then { ZAU_UnitsGarrison = 4 }; // # of units in garrison
