...
for [{_wave = 1}, {_wave < _waveMax}, {_wave = _wave + 1}] do {
	...
	switch (_wave) do {
		case 1: {
			[_location, _spawnRoad, _side, selectRandom _Truck] call zmm_fnc_spawnUnit;
		...
		default {
			[_location, _spawnRoad, _side, selectRandom (_Truck + _Light + _Medium)] call zmm_fnc_spawnUnit;
			[_location, _spawnRoad, _side, selectRandom (_Light + _Medium + _Heavy)] call zmm_fnc_spawnUnit;
			[_location, _spawnLand, _side, 4] call zmm_fnc_spawnUnit;
			[_location, _spawnLand, _side, 4] call zmm_fnc_spawnUnit;
		};
	};
