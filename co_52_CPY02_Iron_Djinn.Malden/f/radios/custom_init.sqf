...
if isServer then {
	...
	f_var_chWest = []; f_var_chEast = []; f_var_chGuer = [];
	{
			...
			if (_customCh != "") then {
				...
				if _newChannel then {
					...
					if (_channelID != 0) then {
						...
						diag_log format ["[F3] ERROR Custom channel '%1' creation failed!", _customCh];
					};
				};
			};
		} forEach _groupList;
		...
		if (count _tempArr == 0 && toUpper (getText ((getMissionConfig "Header") >> "gameType")) == "COOP") then {
...
if hasInterface then {
	...
	if (count _chList > 0) then {
		...
		private _joinText = "";
		{
			...
			// SL and CO automatically get added channels.
			if ((groupId group player in _chGrps && {leader player isEqualTo player}) || rank player == "COLONEL") then {
