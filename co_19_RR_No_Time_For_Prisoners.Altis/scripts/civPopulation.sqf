...
if (isNil "ZCS_var_MaxGrpCount") then { ZCS_var_MaxGrpCount = 5 };
if (isNil "ZCS_var_MinDist") then { ZCS_var_MinDist = 100 };
if (isNil "ZCS_var_MaxDist") then { ZCS_var_MaxDist = 300 };
if (isNil "ZCS_var_EnemyChance") then { ZCS_var_EnemyChance = 0.5 }; // Chance of a hostile civilian appearing
...
if (isNil "ZCS_var_LOWTasks") then { ZCS_var_LOWTasks = false };
...
ZCS_fnc_SpawnHunter = {
	...
	uniformContainer _hunter addItemCargo ["HandGrenade", 3];
