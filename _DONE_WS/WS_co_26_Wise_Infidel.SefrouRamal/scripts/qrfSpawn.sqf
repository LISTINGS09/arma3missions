...
_Truck = ["C_Truck_02_transport_F"];
_Light = ["I_Tura_Offroad_armor_armed_lxWS","I_Tura_Offroad_armor_AT_lxWS"];
...
_Air = ["O_Heli_Light_02_unarmed_F"];
...
for [{_wave = 1}, {_wave < _waveMax}, {_wave = _wave + 1}] do {
	...
	switch (_wave) do {
		...
		case 3: {
			...
			[_location, _spawns, _side, selectRandom (_Medium)] call zmm_fnc_spawnUnit;
		};
		case 4: {
			[_location, _spawns, _side, selectRandom (_Light + _Truck)] call zmm_fnc_spawnUnit;
			[_location, _spawns, _side, selectRandom (_Truck + _Medium)] call zmm_fnc_spawnUnit;
		...
		default {
			...
			[_location, _spawns, _side, selectRandom (_Heavy)] call zmm_fnc_spawnUnit;
		};
	};
