...
if(count _uniform > 0) then {
	...
	if (isPlayer _unit) then { missionNamespace setVariable [format["f_var_%1_%2_gear_uniform",_side,_typeofUnit], _uniform select {isClass (configFile >> "CfgWeapons" >> _x)}, true] };
};
...
if(count _helmet > 0) then {
	...
	if (isPlayer _unit) then { missionNamespace setVariable [format["f_var_%1_%2_gear_headgear",_side,_typeofUnit], _helmet select {isClass (configFile >> "CfgWeapons" >> _x)}, true] };
};
...
if(count _rig > 0) then {
	...
	if (isPlayer _unit) then { missionNamespace setVariable [format["f_var_%1_%2_gear_vest",_side,_typeofUnit], _rig] };
};
...
if(count _glasses > 0) then {
	...
	if (isPlayer _unit) then { missionNamespace setVariable [format["f_var_%1_%2_gear_goggles",_side,_typeofUnit], _glasses] };
} else {
	removeGoggles _unit;
};